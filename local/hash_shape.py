...
from arcface_hash import (arcface_keymix_v1, arcface_blend_v2, arcface_keymix_whitened_v3,
                          load_gallery, load_basis, cosine)
...
def _transform_for(key, args, aux):
    """Build raw512 -> emb512 for one key. key in ('','raw') -> identity (un-hashed).
    aux = gallery (blend_v2) or basis (whitened_v3), loaded once in main()."""
    if key in ("", "raw", None):
        return lambda e: np.asarray(e, np.float32)
    if args.transform == "arcface_keymix_whitened_v3":
        return lambda e: arcface_keymix_whitened_v3(e, key, aux, args.offset)
    if args.transform == "arcface_blend_v2":
        return lambda e: arcface_blend_v2(e, key, aux, args.strength, args.n_mix)
...
def main():
    ...
    ap.add_argument("--transform",
                    choices=["arcface_keymix_v1", "arcface_blend_v2", "arcface_keymix_whitened_v3"],
    ...
    ap.add_argument("--column", default="mica", help="gallery/basis column for the MICA geometry path")
    ...
    if a.transform == "arcface_blend_v2":
        ...
        aux = load_gallery(a.gallery, a.column)
    elif a.transform == "arcface_keymix_whitened_v3":
        if not a.basis:
            raise SystemExit("arcface_keymix_whitened_v3 needs --basis (build one with build_basis.py)")
        aux = load_basis(a.basis, a.column)
    ...
    shapes = {}
    for k in a.keys:
        s = shape_for(h, codedict, raw, _transform_for(k, a, aux))
