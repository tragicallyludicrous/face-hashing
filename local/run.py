"""
run.py — one command, end-to-end local pipeline: photos -> per-person {<stem>_arcface.npy, <stem>_shape.glb, <stem>_composed.glb}.

    python run.py -i in -o out

For every image in <in>, it writes out/<stem>/ containing three files:
  1. <stem>_arcface.npy   — the 512-d ArcFace recognition embedding (the identity / Stage-2 key)
  2. <stem>_shape.glb     — MICA's neutral mesh: identity/shape only, no expression or pose
  3. <stem>_composed.glb  — MICA shape (identity) wearing SMIRK's expression+pose from this same photo
...
def _stage(title, cmd):
    print(
        f"\n=== {title} ===", flush=True
    )  # flush so headers stay ordered vs child output
...
def run(in_dir, out_dir, device="cpu", detector="antelopev2", keep_work=False):
    ...
    try:
        ...
        _stage(
            "MICA — identity (ArcFace 512-d + FLAME shape 300-d)",
            [
                PY,
                os.path.join(HERE, "mica_local.py"),
                "-i",
                in_dir,
                "-o",
                mica_out,
        ...
        _stage(
            "SMIRK — expression/pose + compose (MICA shape + SMIRK pose)",
            [
                PY,
                os.path.join(HERE, "smirk_local.py"),
                "-i",
                in_dir,
                "-o",
                smirk_out,
        ...
        # Assemble the clean deliverable: out/<stem>/{<stem>_arcface.npy, <stem>_shape.glb, <stem>_composed.glb}
        ...
        for stem in sorted(os.listdir(mica_out)) if os.path.isdir(mica_out) else []:
            ...
            shape = os.path.join(mica_out, stem, f"{stem}.glb")  # MICA neutral mesh: shape/identity only
            glb = os.path.join(smirk_out, stem, "composed.glb")
            if not os.path.exists(arc):
                continue
            if not os.path.exists(glb):
                print(
                    f"  {stem}: arcface only — SMIRK produced no composed.glb (skipped)"
                )
                continue
            d = os.path.join(out_dir, stem)
            os.makedirs(d, exist_ok=True)
            shutil.copy2(arc, os.path.join(d, f"{stem}_arcface.npy"))
            shutil.copy2(glb, os.path.join(d, f"{stem}_composed.glb"))
            out_files = [f"{stem}_arcface.npy", f"{stem}_composed.glb"]
            if os.path.exists(shape):  # the shape-only (neutral identity) mesh from MICA
                shutil.copy2(shape, os.path.join(d, f"{stem}_shape.glb"))
                out_files.insert(1, f"{stem}_shape.glb")
            print(f"  {stem}: " + " + ".join(out_files))
            stems.append(stem)
        return stems
    finally:
        if keep_work:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Photos -> per-person {<stem>_arcface.npy, <stem>_shape.glb, <stem>_composed.glb} (MICA identity + SMIRK pose)."
    )
    ...
    p.add_argument(
        ...
        "--out_dir",
        required=True,
        help="output folder (writes <stem>/ per photo)",
    )
    p.add_argument(
        "--device",
        default=os.environ.get("FACE_DEVICE", "cpu"),
        help="cpu (default) | mps | auto",
    )
    p.add_argument(
        "--detector",
        default="antelopev2",
        choices=["antelopev2", "vision"],
        help="MICA's face detector: antelopev2 (default) | vision (macOS, via loupe)",
    )
    p.add_argument(
        "--keep-work",
        action="store_true",
        help="keep the temp dir of intermediates (debug)",
    )
    a = p.parse_args()
    names = run(
        a.in_dir, a.out_dir, device=a.device, detector=a.detector, keep_work=a.keep_work
    )
